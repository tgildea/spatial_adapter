    ...
    def self.get_conditions(attrs)
      attrs.map do |attr, value|
        if columns_hash[attr].is_a?(SpatialColumn)
          ...
          conditions << "MBRIntersects(?, #{table_name}.#{connection.quote_column_name(attr)}) "
        else
          #original stuff
  ...
  def columns(table_name, name = nil)#:nodoc:
    table_structure(table_name).map do |field|
      if field['type'] =~ /geometry|point|linestring|polygon|multipoint|multilinestring|multipolygon|geometrycollection/i
        ActiveRecord::ConnectionAdapters::SpatialSQLiteColumn.new(field['name'], field['dflt_value'], field['type'], field['notnull'] == "0")
      else
